...
export const signup = async (req, res, next) => {
  ...
  return await createNewUser(req, res, next);
};
// 2]==================== Confirm Email =======================
export const confirmEmail = async (req, res, next) => {
  ...
  try {
    await sendEmail({
      to: email,
      subject: 'Welcome to Coursera Clone',
...
export const createDefaultAdmins = async () => {
  const defaultAdmins = [
    {email: 'admin1@example.com', password: '123456', role: 'Admin', isConfirmed: true},
    {email: 'admin2@example.com', password: 'abcdef', role: 'Admin', isConfirmed: true},
    {email: 'admin3@example.com', password: '1z3y5x', role: 'Admin' , isConfirmed: true},
  ];
  try {
    for (const admin of defaultAdmins) {
      ...
      if (!existingAdmin) {
        ...
        const newAdmin = new userModel({email: admin.email, password: admin.password, username: admin.email.split('@')[0], role: admin.role, isConfirmed: admin.isConfirmed});
        await newAdmin.save();
